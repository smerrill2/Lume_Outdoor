...
const JobberFormEmbed = ({ formId = 'homepage-jobber-form' }) => {
  ...
  useEffect(() => {
    ...
    if (!existingScript) {
      ...
      if (typeof window !== 'undefined' && window.Jobber && window.Jobber.requestForms) {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (submitButton && !submitButton.hasAttribute('data-conversion-tracked')) {
        ...
        // A button click is intent, not a verified lead submission.
        submitButton.addEventListener('click', (e) => {
          if (typeof window.gtag_report_lead_start === 'function') {
            window.gtag_report_lead_start('legacy_jobber_embed');
          }
